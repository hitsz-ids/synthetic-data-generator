...
def test_parallel_transform_unfixed():
    transformer, data_loader = preparing_data()
    # start test
    ndarry_loader = unfixed_parallel_transform(
        transformer, data_loader, transformer._column_transform_info_list
    )
    data_sampler = DataSampler(ndarry_loader, transformer.output_info_list, True)
...
def test_parallel_transform_unfixed_caused_columns_switching():
    ...
    unfixed_ndarry_loader = unfixed_parallel_transform(
        unfixed_transformer, unfixed_data_loader, unfixed_transformer._column_transform_info_list
    )

    find_not_matching_column_type(
        unfixed_ndarry_loader, unfixed_transformer._column_transform_info_list
    )
