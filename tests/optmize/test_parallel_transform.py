...
def preparing_data():
    ...
    data_metadata.datetime_format = {
        key: "%Y/%m/%d" for key in data_metadata.datetime_columns
    }
...
def find_not_matching_column_type(data, column_transform_info_list):
    ...
    for column_transform_info in column_transform_info_list:
        ...
        if column_transform_info.column_type == 'discrete':
            ...
            print(f"Filter not one-hot data for column {column_transform_info.column_name}: ",
                  arr[(arr != 0) & (arr != 1)])
